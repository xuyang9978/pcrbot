...
public class UnionBattlePlugin extends CQPlugin {
    ...
    public void queryBossStatus(CoolQ cq, CQGroupMessageEvent event) {
        ...
        // 获取当前boss挂树中的成员列表
        List<TreeList> treeLists = treeListService.findCurrentTreeList(groupID, whichOne, rounds);
        // 获取正在打当前boss的成员列表
        ...
        if (parameters.length != 2) {
            ...
            if (BotConstants.MAX_BATTLE_DAY == battleLogService.countDayLogByUserID(userID, groupID)) {
                ...
                if (null != battlingListService.findBattlingListByUserID(userID, groupID)) {
                    ...
                    } else if (null != treeListService.findCurrentTreeListByUserID(userID, groupID, BotConstants.ON_TREE)) {
                        // 先修改当前挂树记录为下树了
                ...
                if (null == memberService.findMemberByUserID(ateduserID, groupID)) {
                    ...
                    if (BotConstants.MAX_BATTLE_DAY == battleLogService.countDayLogByUserID(ateduserID, groupID)) {
                        ...
                        if (null != battlingListService.findBattlingListByUserID(ateduserID, groupID)) {
                            ...
                            } else if (null != treeListService.findCurrentTreeListByUserID(ateduserID, groupID, BotConstants.ON_TREE)) {
                                // 先修改当前挂树记录为下树了
                                int status = BotConstants.DOWN_TREE;
    ...
    public void adjustBossStatus(CoolQ cq, CQGroupMessageEvent event, String[] msg) {
        ...
        if (parameters.length != 3) {
            ...
            int curWhichOne = Integer.parseInt(parameters[1]);
            int curHP = Integer.parseInt(parameters[2]);
            ...
            if (curWhichOne != whichOne) {
                cq.sendGroupMsg(groupID, "修改的boss编号错误，当前正在打第" + rounds + "周目的老" + whichOne + "！", false);
            } else {
                // 判断更新的是哪一只boss
                switch (curWhichOne) {
                    case 1:
                        ...
                        bossListService.updateBoss1Status(groupID, curHP, curWhichOne);
                        break;
                    case 2:
                        if (curHP == 0) {
                            curWhichOne++;
                        }
                        bossListService.updateBoss2Status(groupID, curHP, curWhichOne);
                        break;
                    case 3:
                        if (curHP == 0) {
                            curWhichOne++;
                        }
                        bossListService.updateBoss3Status(groupID, curHP, curWhichOne);
                        break;
                    case 4:
                        if (curHP == 0) {
                            curWhichOne++;
                        }
                        bossListService.updateBoss4Status(groupID, curHP, curWhichOne);
                        break;
                    case 5:
